/*
 * My NHS Graph
 *
 * WIP Strawman API ideas for a Unified API myNHS API Graph endpoint, bringing together both new \"User Generated Data\" and existing sources into one endpoint and one Authorisation model, supporting delegated user access and proxy access. Modeling that the records belong to the \"tennant\" with owners having proxy access (ie full) and users being shared specifics records. Oauth scopes, with modifiers of .shared and .all. Look at Microsoft Graph for where that comes from. To support idempotency, all POST apis are just requests to start something, the put starts this. Alows for evential consistency. Long running GETs are removed, by POSTing requests for data, with a GET endpoint returned.
 *
 * OpenAPI spec version: 0.1
 * 
 * Generated by: https://github.com/swagger-api/swagger-codegen.git
 */
using System;
using System.Linq;
using System.IO;
using System.Text;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.ComponentModel.DataAnnotations;
using System.Runtime.Serialization;
using Newtonsoft.Json;

namespace mynhs.Models
{ 
    /// <summary>
    /// 
    /// </summary>
    [DataContract]
    public partial class SubmissionResponseDetails : IEquatable<SubmissionResponseDetails>
    { 
        /// <summary>
        /// Gets or Sets SystemName
        /// </summary>

        [DataMember(Name="SystemName")]
        public string SystemName { get; set; }

        /// <summary>
        /// Gets or Sets Status
        /// </summary>

        [DataMember(Name="Status")]
        public StatusEnum Status { get; set; }

        /// <summary>
        /// Gets or Sets IsCompletedSuccessfully
        /// </summary>

        [DataMember(Name="IsCompletedSuccessfully")]
        public bool? IsCompletedSuccessfully { get; set; }

        /// <summary>
        /// Gets or Sets PercentComplete
        /// </summary>

        [DataMember(Name="PercentComplete")]
        public int? PercentComplete { get; set; }

        /// <summary>
        /// Gets or Sets DateStarted
        /// </summary>

        [DataMember(Name="DateStarted")]
        public DateTimeOffset? DateStarted { get; set; }

        /// <summary>
        /// Gets or Sets DateFinished
        /// </summary>

        [DataMember(Name="DateFinished")]
        public DateTimeOffset? DateFinished { get; set; }

        /// <summary>
        /// Gets or Sets DateUpdated
        /// </summary>

        [DataMember(Name="DateUpdated")]
        public DateTimeOffset? DateUpdated { get; set; }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            var sb = new StringBuilder();
            sb.Append("class SubmissionResponseDetails {\n");
            sb.Append("  SystemName: ").Append(SystemName).Append("\n");
            sb.Append("  Status: ").Append(Status).Append("\n");
            sb.Append("  IsCompletedSuccessfully: ").Append(IsCompletedSuccessfully).Append("\n");
            sb.Append("  PercentComplete: ").Append(PercentComplete).Append("\n");
            sb.Append("  DateStarted: ").Append(DateStarted).Append("\n");
            sb.Append("  DateFinished: ").Append(DateFinished).Append("\n");
            sb.Append("  DateUpdated: ").Append(DateUpdated).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }

        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public string ToJson()
        {
            return JsonConvert.SerializeObject(this, Formatting.Indented);
        }

        /// <summary>
        /// Returns true if objects are equal
        /// </summary>
        /// <param name="obj">Object to be compared</param>
        /// <returns>Boolean</returns>
        public override bool Equals(object obj)
        {
            if (ReferenceEquals(null, obj)) return false;
            if (ReferenceEquals(this, obj)) return true;
            return obj.GetType() == GetType() && Equals((SubmissionResponseDetails)obj);
        }

        /// <summary>
        /// Returns true if SubmissionResponseDetails instances are equal
        /// </summary>
        /// <param name="other">Instance of SubmissionResponseDetails to be compared</param>
        /// <returns>Boolean</returns>
        public bool Equals(SubmissionResponseDetails other)
        {
            if (ReferenceEquals(null, other)) return false;
            if (ReferenceEquals(this, other)) return true;

            return 
                (
                    SystemName == other.SystemName ||
                    SystemName != null &&
                    SystemName.Equals(other.SystemName)
                ) && 
                (
                    Status == other.Status ||
                    Status != null &&
                    Status.Equals(other.Status)
                ) && 
                (
                    IsCompletedSuccessfully == other.IsCompletedSuccessfully ||
                    IsCompletedSuccessfully != null &&
                    IsCompletedSuccessfully.Equals(other.IsCompletedSuccessfully)
                ) && 
                (
                    PercentComplete == other.PercentComplete ||
                    PercentComplete != null &&
                    PercentComplete.Equals(other.PercentComplete)
                ) && 
                (
                    DateStarted == other.DateStarted ||
                    DateStarted != null &&
                    DateStarted.Equals(other.DateStarted)
                ) && 
                (
                    DateFinished == other.DateFinished ||
                    DateFinished != null &&
                    DateFinished.Equals(other.DateFinished)
                ) && 
                (
                    DateUpdated == other.DateUpdated ||
                    DateUpdated != null &&
                    DateUpdated.Equals(other.DateUpdated)
                );
        }

        /// <summary>
        /// Gets the hash code
        /// </summary>
        /// <returns>Hash code</returns>
        public override int GetHashCode()
        {
            unchecked // Overflow is fine, just wrap
            {
                var hashCode = 41;
                // Suitable nullity checks etc, of course :)
                    if (SystemName != null)
                    hashCode = hashCode * 59 + SystemName.GetHashCode();
                    if (Status != null)
                    hashCode = hashCode * 59 + Status.GetHashCode();
                    if (IsCompletedSuccessfully != null)
                    hashCode = hashCode * 59 + IsCompletedSuccessfully.GetHashCode();
                    if (PercentComplete != null)
                    hashCode = hashCode * 59 + PercentComplete.GetHashCode();
                    if (DateStarted != null)
                    hashCode = hashCode * 59 + DateStarted.GetHashCode();
                    if (DateFinished != null)
                    hashCode = hashCode * 59 + DateFinished.GetHashCode();
                    if (DateUpdated != null)
                    hashCode = hashCode * 59 + DateUpdated.GetHashCode();
                return hashCode;
            }
        }

        #region Operators
        #pragma warning disable 1591

        public static bool operator ==(SubmissionResponseDetails left, SubmissionResponseDetails right)
        {
            return Equals(left, right);
        }

        public static bool operator !=(SubmissionResponseDetails left, SubmissionResponseDetails right)
        {
            return !Equals(left, right);
        }

        #pragma warning restore 1591
        #endregion Operators
    }
}
